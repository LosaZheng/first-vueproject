...
<!-- 我们可以 使用 watch 属性监听，来 监听 父组件传递过来的 max 值，不管 watch 会被触发几次，但是，最后一次，肯定是一个 合法的 max 数值 -->
  <div class="mui-numbox" data-numbox-step='1' data-numbox-min='1' >
    ...
    <!-- change事件监听值得变化 -->
    <input id="test" class="mui-input-numbox" type="number" value="1" ref="count" @change="getCount">
...
export default {
  ...
  mounted() {
    ...
    //页面加载完成后为undefined,因为父组件请求数据时，是异步对象，请求的数据可能还没获取到
    // console.log(this.max);
  },
  methods:{
    getCount(){
      // console.log(parseInt(this.$refs.count.value));
